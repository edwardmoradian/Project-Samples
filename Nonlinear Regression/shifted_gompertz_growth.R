...
curve_model_data <- read_csv("data/modeldata.csv",col_names = TRUE)
...
model_sgompertz <- nlsLM(MED_EST_BI_8~model_fn(m,b,eta,t),data=curve_model_train_data,start=list(m=m,b=b,eta=eta),
...
# In the residuals vs. fitted vales visualization
...
viz <- data.frame("BI_EST_8_PRED"=predictions,t=seq(1:length(predictions)))
viz_2 <- left_join(curve_model_data,viz,by="t")

value_col <- c("red","blue","blue","black")
value_col_2 <- c("Forecast", "BI_EST_8_Line", "BI_EST_8_Points", "Forecast New Data")

gg <- ggplot(viz_2) +
  labs(title="Shifted-Gompertz Fit Median",xlab("t"),ylab("EST_BI_8")) +
  geom_line(aes(x = t, y = BI_EST_8_PRED,color="1")) +
  geom_point(aes(x = t, y = BI_EST_8,color="2")) +
  geom_line(aes(x = t, y = BI_EST_8,color="3")) +
...
model_sgompertz_2 <- nlsLM(MED_EST_BI_8~model_fn(b,eta,t),data=curve_model_train_data,start=list(b=b,eta=eta),
                         algorithm = "LM",lower=bounds, trace=TRUE,control = list(maxiter = maxiter, gtol=converge))
